...
void ProjectileGame::viewBallRoute(SDL_Renderer* renderer)
{
	Ball* ball = balls_[num_of_balls_ - 1];
	double mouse_game_x = W2G_X(mouse_win_x_);
	double mouse_game_y = W2G_Y(mouse_win_y_);
	...
	ball->viewRouteOfBall(renderer, ball_texture_, ball_src_rectangle_, launcing_force_x, launcing_force_y);
}
